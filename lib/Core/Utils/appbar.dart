            ...
            Obx(
              () => CustomButton(
                ...
                ontap: () async {
                  ...
                  await backDoorFtnBtn.getdata();
                  await printIdController.poitnOfSaleMaterailDB();

                  print("here is  outer data ===>> ${printIdController.productDetails}");
                  print("here is  outer competitorPromotion data ===>> ${printIdController.competitorPromotion}");
                  
                  
                  List convertedList = printIdController.productDetails.values.map((innerMap) => innerMap.cast<String, dynamic>())
                      .toList();
                  List convertedListPL = printIdController.priceLabel.values.map((innerMap) => innerMap.cast<String, dynamic>())
                      .toList();
                  List convertedListPS = printIdController.promotionSecondary.values.map((innerMap) => innerMap.cast<String, dynamic>())
                      .toList();
                  List convertedListCM = printIdController.competitorPromotion.values.map((innerMap) => innerMap.cast<String, dynamic>())
                      .toList();
                  List convertedListpointOFsales = printIdController.pointOfSaleMaterial.values.map((innerMap) => innerMap.cast<String, dynamic>())
                      .toList();
                  List  list = backDoorFtnBtn.backDoor.values.map((innerMap) => innerMap.cast<String, dynamic>())
                      .toList();
